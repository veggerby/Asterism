<!--
GitHub Copilot / AI Contributor Instructions for the Asterism codebase.
These rules are binding for any AI-generated contribution.
-->

# Asterism – AI Contribution Guidelines

Asterism is a production-focused .NET framework/library. All AI assistance must produce code that is correct, deterministic, maintainable, and aligned with existing architectural principles. Humor or playful tone from other templates is not appropriate here.
...
- Explicit namespaces mirroring folders (`Asterism.Time`, `Asterism.Coordinates`, etc.).
...
- Metrics: use `IAsterismMetrics`; default is no-op; do not depend on concrete implementation.
...
Every new core component must include unit tests under the appropriate `tests/Asterism.*` project using xUnit.
...
- `ServiceCollectionExtensions.AddAsterism` is the single composition entry point—extend via options before adding new overloads.
...
---
## 15. Final Reminder
If an AI suggestion increases complexity without measurable benefit (performance, clarity, extensibility), **reject it**. Asterism prioritizes stable, predictable infrastructure primitives over novelty.
